...
`repo/`, `patches/`, `Dockerfile`, `Makefile` and `scripts/` were deleted once
Debian was running and committed on hardware. Two things follow, and they are different:
...
* **The build is gone.** Nothing in this repository can produce an
  ...
  ```sh
  # ${R} braced, and ~1 rather than ^: a bare $R:path is a modifier in zsh and
  # ^ is a glob character there, so the obvious spelling fails for zsh users.
  R="$(git log --diff-filter=D --format=%H -1 -- Makefile)~1"
  git show "${R}:repo/meta-apollo/conf/machine/meson-apollo.conf"  # read one file
  git checkout "${R}" -- repo patches Dockerfile Makefile scripts  # restore the tree
  ```
...
path, the `mender_*` artefact names). Those citations still resolve in history
with `git show`, and they remain the evidence for *why* the current design is
...
* **Looking for the old Yocto build?** [Above](#the-yocto-tree) — it is in the
  history, not gone.
