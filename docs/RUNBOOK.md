...
#    e2fsprogs-mke2fs only, and none of the Phase 4 gate tools. Build them
#    from the bitbake tree. If that tree has already been removed from this
#    repository, restore it from history first ($R empty means it is still here):
# R="$(git log --diff-filter=D --format=%H -1 -- Makefile)"
# [ -n "$R" ] && git checkout "${R}~1" -- repo patches Dockerfile Makefile scripts
