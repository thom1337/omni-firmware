...
| **2** | Armbian kernel factory | `armbian/` submodule pinned to a SHA; `board/` synced in via one-way rsync | `./compile.sh kernel BOARD=gateway-gz80x BRANCH=<pinned>` produces four `-meson64` debs. Proves the host, not your DTS. | 1 | n/a |
| **3** | Author the DTS | `meson-axg-apollo.dts`, `avast-omni.csc`, kernel config delta, `docs/dts-port-audit.md` | `./compile.sh dts-check` clean; committed node-for-node diff of `dtc -I dtb -O dts` old vs new, every delta classified present / deleted / moved-to-dtsi | 3 | n/a |
| **4** | **GO/NO-GO: kernel on hardware** | Slot B = byte-clone of slot A + new `Image`/`dtb`/`lib/modules` | See gate below | 2 | Power cycle (nothing persistent written) |
...
| **7** | Debian rootfs + initramfs | `omni-slot.ext4.gz` | qemu-aarch64 loopback boot reaches login on a merged overlay | 4 | n/a |
...
> **Kernel version — re-checked 2026-08-02:** 6.12 is now Armbian's `oldlts` and its projected EOL (Dec 2026) is only months away. 6.18 LTS — Armbian `current`, archive dir `patch/kernel/archive/meson64-6.18/` — is supported into late 2027 and is probably the better pin. Everything in this plan transfers unchanged; on 6.18 three details actually get simpler: `default-brightness` (landed in 6.13) works, the PWM node is already the pwm-v2 binding described below, and the drop directory becomes `meson64-6.18/dt/`. Armbian's meson64 kernel config is identical on both branches for every option this plan touches (verified against `linux-meson64-oldlts.config` and `-current.config`).
...
**Reset button:** GPIOAO_10, active low. U-Boot reads it as a raw register (`setexpr button *0xff800028 & 0x400`, `.../0047-recovery-apollo-Check-registers-to-detect-button.patch`) and is unaffected by Linux. In the DTS add `gpio-keys` with `<&gpio_ao GPIOAO_10 GPIO_ACTIVE_LOW>` → `KEY_RESTART`; `CONFIG_KEYBOARD_GPIO=y` in stock Armbian.

**Kernel config delta** (`userpatches/config/kernel/linux-meson64-<branch>.config`), asserted by `tools/check-kconfig-invariants.sh` in CI:
