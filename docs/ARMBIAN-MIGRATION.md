...
| P6 | Reset-button cold boot reaches a usable shell on p7 | **Superseded.** This repo now builds a Debian recovery image for p7 (`rootfs/overlay-recovery`, 450 MiB, no initrd, `init=/init`) — see [`RUNBOOK.md` Appendix D](RUNBOOK.md#appendix-d--the-recovery-slot-p7). It is still true that nothing here builds `apollo-mfc-initrd-image-*`, so **`force_run_mfc=1` remains fatal**, and that overwriting p7 destroys the factory MFC payload irreversibly — take the whole-eMMC backup first. If p7 is empty, recovery is serial-only until you write one. |
...
| **4** | **GO/NO-GO: kernel on hardware** | Slot B = byte-clone of slot A + new `Image`/`dtb`/`lib/modules` | 🟡 **SUBSTANTIALLY PASSED 2026-08-02; only cable-dependent items remain.** Two trial boots of `6.18.41` with our DTS, entirely from the `=>` prompt against slot A's own userland — nothing persistent written, plain reboot returns to 5.4.62 each time. **Passed:** `Machine model: Avast Omni (Apollo)`; eMMC **HS200, 200 MHz, 8-bit** (exact 5.4 match); all 7 partitions; overlay root; login shell; LEDs exactly `apollo:power`/`app1`/`app2`; `gpio-keys-polled` registered; no `meson_nand`. **Risk #3 largely answered:** MAC probes (DWMAC1000), **`REALTEK_PHY=y` binds the real RTL8211F driver rather than degrading to genphy**, the PHY interrupt is genuine (`/proc/interrupts`: `meson-gpio-irqchip 98 Level stmmac-0:00`), and the `ethernet0` alias delivers `ethaddr` to `eth0` byte-exact. **Still open (needs a cable + `ethtool`/`iperf3`):** link, per-direction throughput vs 5.4, cable-pull interrupt counts, and the MDIO 0xd08 delay comparison deciding `rgmii` vs `rgmii-rxid`. **Also open:** no thermal zones — this unit's SCP firmware is `legacy pre-1.0`, so `scpi_sensors` does not exist. Detail in `docs/HARDWARE-MEASURED.md`. | 2 | Power cycle (nothing persistent written) |
